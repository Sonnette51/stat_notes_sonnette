...
> [!ABSTRACT] 本讲概要
> 本讲开始进入语音识别（Automatic Speech Recognition, ASR）。核心不是马上讲模型，而是先把问题写清楚：语音输入如何表示？文字输出如何表示？为什么输入长度通常远大于输出长度？不同输出词元/符号（token），例如音素（phoneme）、字素（grapheme）、词（word）、语素（morpheme）、字节（byte），各有什么代价？语音特征又为什么常用滤波器组特征（filter bank features, fbank）或梅尔频率倒谱系数（Mel-frequency cepstral coefficients, MFCC），而不是直接用原始波形（waveform）？
>
> 课程也提醒我们：ASR 今天看起来理所当然，但历史上曾被认为非常困难。这个背景有助于理解为什么本讲要认真讨论 token、alignment、feature 和 data，而不是直接跳进深度模型。
>
> 本讲最后给出后续几讲的模型地图：听-注意-拼写模型（Listen, Attend and Spell, LAS）、连接时序分类（Connectionist Temporal Classification, CTC）、RNN 转导器（Recurrent Neural Network Transducer, RNN-T）、神经转导器（Neural Transducer）、单调分块注意力（Monotonic Chunkwise Attention, MoChA）和 DNN-HMM 混合模型（DNN-HMM hybrid model）。理解这些模型前，必须先理解本讲的输入输出设定。
>
> ---
